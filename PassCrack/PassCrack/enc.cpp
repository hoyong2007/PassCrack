#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#pragma warning(disable:4996)
/*
extern char passwd_txt[128];
extern char passwd_enc[128];
*/


int validate_()
{
	char _passwd_txt[128];
	char _passwd_enc[256];
	char _random_str[128];
	char key1[128] = {
		
	};
	char key2[128] = {
		
	};
	char passwd_txt_N[256];
	char passwd_enc_N[256];
	char tmp[256];
	int i;
	//decrypt(str, pass_len);
	for (i = 0; i<128; i++)
		tmp[i] = _passwd_enc[i] ^ key2[i];

	for (i = 0; i < 128; i++) {
		passwd_txt_N[i] = tmp[i] ^ _passwd_enc[i + 128];
		passwd_txt_N[i + 128] = _passwd_enc[i];
	}

	for (i = 0; i < 128; i++) {
		tmp[i] = passwd_txt_N[i] ^ key1[i];
		tmp[i + 128] = passwd_txt_N[i];
	}

	for (i = 0; i < 128; i++) {
		passwd_txt_N[i] = tmp[i] ^ passwd_txt_N[i + 128];
		passwd_txt_N[i + 128] = tmp[i + 128];
	}

	for (i = 0; i < 128; i++) {
		if (_passwd_txt[i] != passwd_txt_N[i])
			return 0;
	}
	return 1;
	//------
}


char* Change_()
{
	char _passwd_txt[128];
	char _passwd_enc[256];
	char _random_str[128];
	char key1[128] = {
		
	};
	char key2[128] = {
		
	};
	char random_str[128];
	char passwd_enc_N[256];
	char tmp[256];
	int i;
	//encrypt(str, random_str, pass_len);
	for (i = 0; i<128; i++)
		tmp[i] = random_str[i] ^ key1[i];

	for (i = 0; i < 128; i++) {
		passwd_enc_N[i] = random_str[i];
		passwd_enc_N[i + 128] = tmp[i] ^ _passwd_txt[i];
	}

	for (i = 0; i < 128; i++) {
		tmp[i] = passwd_enc_N[i + 128] ^ key2[i];
		tmp[i + 128] = passwd_enc_N[i + 128];
	}

	for (i = 0; i < 128; i++) {
		passwd_enc_N[i + 128] = tmp[i] ^ passwd_enc_N[i];
		passwd_enc_N[i] = tmp[i + 128];
	}

	return passwd_enc_N;
}
